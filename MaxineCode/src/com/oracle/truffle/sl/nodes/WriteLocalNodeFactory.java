...
public final class WriteLocalNodeFactory implements NodeFactory<WriteLocalNode> {
    ...
    private abstract static class WriteLocalBaseNode extends WriteLocalNode {
        ...
        protected Object executeAndSpecialize0(int minimumState, VirtualFrame frameValue, Object rightNodeValue, String reason) {
            neverPartOfCompilation();
            WriteLocalBaseNode current = this;
            String message = createInfo0(reason, frameValue, rightNodeValue);
            if (minimumState < 3) {
                if (minimumState < 1 && SLTYPES.isInteger(rightNodeValue)) {
                    int rightNodeValueCast = SLTYPES.asInteger(rightNodeValue);
                    try {
                        current = current.replace(new WriteLocalIntNode(current), message);
                        return current.write(frameValue, rightNodeValueCast);
                    } catch (FrameSlotTypeException rewriteEx) {
                        // fall through
                    }
                }
                if (minimumState < 2 && SLTYPES.isBoolean(rightNodeValue)) {
                    boolean rightNodeValueCast = SLTYPES.asBoolean(rightNodeValue);
                    try {
                        current = current.replace(new WriteLocalBooleanNode(current), message);
                        return current.write(frameValue, rightNodeValueCast);
                    } catch (FrameSlotTypeException rewriteEx) {
                        // fall through
                    }
                }
                return current.replace(new WriteLocalObjectNode(current), message).writeGeneric(frameValue, rightNodeValue);
            }
            throw new UnsupportedOperationException(createInfo0("Unsupported values", frameValue, rightNodeValue));
            // unreachable Generic
        }
    ...
    private static final class WriteLocalUninitializedNode extends WriteLocalBaseNode {
        ...
        public Object executeGeneric(VirtualFrame frameValue) {
            ...
            return super.executeAndSpecialize0(0, frameValue, rightNodeValue, "Uninitialized monomorphic");
        }
    ...
    private static final class WriteLocalIntNode extends WriteLocalBaseNode {
        ...
        public int executeInteger(VirtualFrame frameValue) throws UnexpectedResultException {
            ...
            try {
                ...
                transferToInterpreter();
                return SLTYPES.expectInteger(executeAndSpecialize0(1, frameValue, ex.getResult(), "Expected rightNodeValue instanceof int"));
            }
            try {
                return super.write(frameValue, rightNodeValue);
            } catch (FrameSlotTypeException ex) {
                transferToInterpreter();
                return SLTYPES.expectInteger(executeAndSpecialize0(1, frameValue, rightNodeValue, "Thrown FrameSlotTypeException"));
            }
        }
        ...
        public Object executeEvaluated(VirtualFrame frameValue, Object rightNodeValueEvaluated) {
            ...
            try {
                ...
                transferToInterpreter();
                return executeAndSpecialize0(1, frameValue, ex.getResult(), "Expected rightNodeValue instanceof int");
            }
            try {
                return super.write(frameValue, rightNodeValue);
            } catch (FrameSlotTypeException ex) {
                transferToInterpreter();
                return executeAndSpecialize0(1, frameValue, rightNodeValue, "Thrown FrameSlotTypeException");
            }
        }
    ...
    private static final class WriteLocalBooleanNode extends WriteLocalBaseNode {
        ...
        public boolean executeBoolean(VirtualFrame frameValue) throws UnexpectedResultException {
            ...
            try {
                ...
                transferToInterpreter();
                return SLTYPES.expectBoolean(executeAndSpecialize0(2, frameValue, ex.getResult(), "Expected rightNodeValue instanceof boolean"));
            }
            try {
                return super.write(frameValue, rightNodeValue);
            } catch (FrameSlotTypeException ex) {
                transferToInterpreter();
                return SLTYPES.expectBoolean(executeAndSpecialize0(2, frameValue, rightNodeValue, "Thrown FrameSlotTypeException"));
            }
        }
        ...
        public Object executeEvaluated(VirtualFrame frameValue, Object rightNodeValueEvaluated) {
            ...
            try {
                ...
                transferToInterpreter();
                return executeAndSpecialize0(2, frameValue, ex.getResult(), "Expected rightNodeValue instanceof boolean");
            }
            try {
                return super.write(frameValue, rightNodeValue);
            } catch (FrameSlotTypeException ex) {
                transferToInterpreter();
                return executeAndSpecialize0(2, frameValue, rightNodeValue, "Thrown FrameSlotTypeException");
            }
        }
