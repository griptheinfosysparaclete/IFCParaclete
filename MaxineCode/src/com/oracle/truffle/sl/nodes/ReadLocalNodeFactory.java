...
public final class ReadLocalNodeFactory implements NodeFactory<ReadLocalNode> {
    ...
    private abstract static class ReadLocalBaseNode extends ReadLocalNode {
        ...
        protected Object executeAndSpecialize0(int minimumState, VirtualFrame frameValue, String reason) {
            ...
            ReadLocalBaseNode current = this;
            String message = createInfo0(reason, frameValue);
            if (minimumState < 3) {
                if (minimumState < 1) {
                    try {
                        current = current.replace(new ReadLocalIntNode(current), message);
                        return current.doInteger(frameValue);
                ...
                if (minimumState < 2) {
                    try {
                        current = current.replace(new ReadLocalBooleanNode(current), message);
                        return current.doBoolean(frameValue);
                    } catch (FrameSlotTypeException rewriteEx) {
                        // fall through
                    }
                }
                return current.replace(new ReadLocalObjectNode(current), message).doObject(frameValue);
            }
            throw new UnsupportedOperationException(createInfo0("Unsupported values", frameValue));
            // unreachable Generic
        }
    ...
    private static final class ReadLocalIntNode extends ReadLocalBaseNode {
        ...
        public int executeInteger(VirtualFrame frameValue) throws UnexpectedResultException {
            try {
                ...
                transferToInterpreter();
                return SLTYPES.expectInteger(executeAndSpecialize0(1, frameValue, "Thrown FrameSlotTypeException"));
            }
        }
        ...
        public Object executeEvaluated(VirtualFrame frameValue, Object otherValue0Evaluated) {
            try {
                ...
                transferToInterpreter();
                return executeAndSpecialize0(1, frameValue, "Thrown FrameSlotTypeException");
            }
        }
    ...
    private static final class ReadLocalBooleanNode extends ReadLocalBaseNode {
        ...
        public boolean executeBoolean(VirtualFrame frameValue) throws UnexpectedResultException {
            try {
                ...
                transferToInterpreter();
                return SLTYPES.expectBoolean(executeAndSpecialize0(2, frameValue, "Thrown FrameSlotTypeException"));
            }
        }
        ...
        public Object executeEvaluated(VirtualFrame frameValue, Object otherValue0Evaluated) {
            try {
                ...
                transferToInterpreter();
                return executeAndSpecialize0(2, frameValue, "Thrown FrameSlotTypeException");
            }
        }
