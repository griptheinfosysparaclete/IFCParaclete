...
public final class PrintNodeFactory implements NodeFactory<PrintNode> {
    ...
    private abstract static class PrintBaseNode extends PrintNode {
        ...
        protected void executeAndSpecialize0(int minimumState, VirtualFrame frameValue, Object child0Value, String reason) {
            neverPartOfCompilation();
            String message = createInfo0(reason, child0Value);
            if (minimumState < 4) {
                if (minimumState < 1 && SLTYPES.isInteger(child0Value)) {
                    int child0ValueCast = SLTYPES.asInteger(child0Value);
                    this.replace(new PrintIntNode(this), message).doInt(child0ValueCast);
                    return;
                }
                if (minimumState < 2 && SLTYPES.isBoolean(child0Value)) {
                    boolean child0ValueCast = SLTYPES.asBoolean(child0Value);
                    this.replace(new PrintBooleanNode(this), message).doBoolean(child0ValueCast);
                    return;
                }
                if (minimumState < 3 && SLTYPES.isString(child0Value)) {
                    String child0ValueCast = SLTYPES.asString(child0Value);
                    this.replace(new PrintStringNode(this), message).doString(child0ValueCast);
                    return;
                }
                this.replace(new PrintObjectNode(this), message).doGeneric(child0Value);
                return;
            }
    ...
    private static final class PrintUninitializedNode extends PrintBaseNode {
        ...
        public void executeVoid(VirtualFrame frameValue) {
            ...
            super.executeAndSpecialize0(0, frameValue, child0Value, "Uninitialized monomorphic");
            return;
        }
    ...
    private static final class PrintIntNode extends PrintBaseNode {
        ...
        public void executeVoid(VirtualFrame frameValue) {
            ...
            try {
                ...
                transferToInterpreter();
                executeAndSpecialize0(1, frameValue, ex.getResult(), "Expected child0Value instanceof int");
                return;
            }
    ...
    private static final class PrintBooleanNode extends PrintBaseNode {
        ...
        public void executeVoid(VirtualFrame frameValue) {
            ...
            try {
                ...
                transferToInterpreter();
                executeAndSpecialize0(2, frameValue, ex.getResult(), "Expected child0Value instanceof boolean");
                return;
            }
    ...
    private static final class PrintStringNode extends PrintBaseNode {
        ...
        public void executeVoid(VirtualFrame frameValue) {
            ...
            try {
                ...
                transferToInterpreter();
                executeAndSpecialize0(3, frameValue, ex.getResult(), "Expected child0Value instanceof String");
                return;
            }
