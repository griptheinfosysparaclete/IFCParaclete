...
public class VMAJavaRunScheme extends JavaRunScheme implements JVMTIException.VMAHandler {
    ...
    private static final String[] shortHandlerNames = new String[] {
        "null", "util.Null",
        "cbc", "cbc.h.CBC", "ifc", "ifcadvicehandlers.h.IFC",
    ...
    public static String getHandlerClassName() {
        ...
        if (handlerClassName == null) {
            ...
            Trace.line(0, "handlerName: " + handlerName);
            
            if (handlerName != null) {
                for (int i = 0; i < shortHandlerNames.length; i += 2) {
                    Trace.line(0, "shortHandlerName[" + i + "]: " + shortHandlerNames[i]);
        ...
        Trace.line(0, "handlerClassName: " + handlerClassName);
    ...
    public void initialize(MaxineVM.Phase phase) {
        ...
        if (MaxineVM.isHosted() && phase == MaxineVM.Phase.BOOTSTRAPPING) {
            ...
            if (handlerClassName != null) {
                try {
                    Log.print("VMA: ");
                    Log.println(handlerClassName);
                    adviceHandler = (VMAdviceHandler) Class.forName(handlerClassName).newInstance();
                } catch (Throwable ex) {
                    fail(ex.getClass() + ":   " +   ex.getCause() + ":   " + ex.getMessage() + ": failed to instantiate VMA advice handler class");
                }
